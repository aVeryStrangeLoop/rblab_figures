...
data_l_agg <- aggregate(x=data_l["p4"],by=list(res=data_l$res_f,pop=data_l$pop_f),FUN=function(x) c(mn = mean(x), stdev = sd(x)))
...
p <- ggplot(data_final,aes(x=res,y=mn,group=pop,color=pop)) +
  ...
  theme(axis.text.x = element_text(angle =45)) + ylim(0,0.4)
