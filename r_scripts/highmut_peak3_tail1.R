library(ggplot2)
data <- read.csv("~/wsl_files/rblab/rblab_figures/data/highmut_samples_100/tail_replace/peak3_tail1.csv")
...
p3 <- ggplot(data_sub_sanit,aes(x="",y=mum))  + facet_grid(.~update,switch="both") +
  geom_violin() + geom_boxplot(width=0.2)+
  theme_minimal() +
  xlab("Update") + ylab("Marginal utility of messaging")
p3 + coord_cartesian(ylim=c(-0.1,1.0))
